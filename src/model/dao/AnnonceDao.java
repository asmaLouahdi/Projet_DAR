...
public class AnnonceDao {
	...
	public List<Object> search_annonce(String keyword) {
		...
		/************************chercher une annonce par mot clés ***************************/
	...
	public List<Object> notification(String email){
		...
		Session session = HibernateUtil.openSession();
				Query q = session.createQuery("from Utilisateur user JOIN user.annonces an JOIN an.comments com where user.email=:email and com.state= 0 order by an.date desc")
				.setParameter("email", email);
		List<Object> list = q.list();
		session.close();
		return list;
