...
public class AnnonceDao {
	...
	public void updateCommentState(String email) {
		...
		Utilisateur usr = (Utilisateur) session.createQuery("from Utilisateur as user where user.email = :email")
				.setParameter("email", email).uniqueResult();
		List<Annonce> an = session.createQuery("from Annonce as an where an.user_id=:user").setParameter("user", usr)
				.list();
		for (Annonce m : an) {
			List<ACommentaire> com = session
					.createQuery("from ACommentaire com where com.annonce_id=:annonce and com.state=0")
	...
	public List<Object> notification(String email) {
		...
		Query q = session
				.createQuery(
						"from Utilisateur user JOIN user.annonces an LEFT JOIN an.comments com where user.email=:email and com.state= 0 GROUP BY an.id order by an.date desc")
				.setParameter("email", email);
		List<Object> list = q.list();
